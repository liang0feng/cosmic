...
public class Demo01 extends AbstractFormPlugin implements ClickListener {

    private final static String KEY_ENTRYENTITY = "kded_entryentity";
    private final static String KEY_MYFIELDCONTAINER = "kded_myfieldcontainer";
...
//        EntryType entryType = (EntryType) newMainType.getAllEntities().get(KEY_ENTRYENTITY);
        ...
        entryKeys.add(KEY_ENTRYENTITY);
        EntityType subEntityType = newMainType.getSubEntityType(entryKeys);

        DataEntityPropertyCollection properties = newMainType.getProperties();

        TextProp textProp = new TextProp();
        textProp.setName(KEY_AUTOTEXT2);
        textProp.setDisplayName(new LocaleString(KEY_AUTOTEXT2));
        textProp.setDbIgnore(true);    // 此字段不需到物理表格取数
        textProp.setAlias("");            // 物理字段名


        subEntityType.registerSimpleProperty(textProp);
        DynamicCollectionProperty dynamicCollectionProperty = new DynamicCollectionProperty();
        dynamicCollectionProperty.setItemType(subEntityType);
        dynamicCollectionProperty.setDbIgnore(true);
        dynamicCollectionProperty.setAlias("");
        dynamicCollectionProperty.setDisplayName(new LocaleString("单据体01"));
        dynamicCollectionProperty.setName(KEY_ENTRYENTITY);


        subEntityType.registerSimpleProperty(textProp);
        newMainType.registerCollectionProperty(dynamicCollectionProperty);


        /*// 把新字段，注册到单据体
        subEntityType.registerSimpleProperty(textProp2);
        DynamicCollectionProperty dynamicCollectionProperty = new DynamicCollectionProperty();
        dynamicCollectionProperty.setItemType(subEntityType);*/
